...
public final class PsiMethodUtils {
    ...
    public static void copyMethods(PsiClass source, PsiClass destination) {
        PsiField[] fields = source.getFields();
        List<PsiField> listFields = new ArrayList<>();
        for (PsiField field : fields) {
            boolean ignoreField = isStatic(field);
            if (!ignoreField) {
        ...
        for (PsiMethod method : source.getMethods()) {
            if (!isAssociatedWithAField(method, listFields)) {
